...
if ((-not $NoPush) -and $RepoPath -and (Test-Path -LiteralPath (Join-Path $RepoPath '.git'))) {
    ...
    foreach ($f in @('SPEC.md', 'ROADMAP.md', 'OVERNIGHT-REPORT.md', 'UX-SPEC.md', 'THEMES-SPEC.md', 'FLAVORS-SPEC.md', 'SETTINGS-SPEC.md', 'DISTRIBUTION-SPEC.md', 'REMOVAL-SPEC.md', 'WAGO-BROWSE-SPEC.md', 'APP-UPDATE-SPEC.md', 'GAME-MODE-SPEC.md', 'SETUP-SPEC.md', 'ADOPT-SPEC.md', 'GITHUB-SOURCE-SPEC.md', 'TESTING.md', 'NIGHT-REPORT-2026-09-05.md', 'NIGHT-REPORT-2026-09-07.md')) {
        $s = Join-Path $Source $f
    ...
    try {
        ...
        if ($pending) {
            ...
            # Eric (2026-09-06): he is the repo's only contributor - no co-author trailer.
