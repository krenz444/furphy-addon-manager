...
- **The app** ("Furphy Addon Manager" shortcut) - installed list with authors, versions, update badges, status/filter chips, sorting and keyboard shortcuts; per addon: update, install any specific version, pin/unpin, ignore updates, **roll back** to the previous version, uninstall, open on CurseForge/Wago; dependency and compatibility (patch) warnings; automatic update checks (persisted); light/dark/vaporwave theme; Settings for release channel, auto-update on launch, the optional API key, untracked-folder scan/adopt/delete, diagnostics, logs and backups; "Update & Play".
