...
- **Background updates** - turn on "Update addons in the background" (Settings, or the tray icon's own right-click menu; optional, off by default) and Furphy checks and updates on its own timer with no window to open - it never touches anything while WoW is running.
