...
| `spa` | `tests\spa\Run-SpaHarness.ps1` (always) - a same-origin copy of `ui\` driven headlessly, 47 DOM/behavior checks (this count drifts release to release - see Round 28 below for the latest addition; do not treat any specific number here as load-bearing). `tests\spa\Run-ThemeAudit.ps1` (full-run only) - live-computed WCAG contrast for all 15 themes plus one screenshot per theme into `tests\theme-screenshots\`. | harness only; theme audit is full-only |
