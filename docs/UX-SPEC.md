...
**Found existing addon folders on first run** - Round 46 (2026-09-09): rewritten so the app matches `install.ps1`'s own Round 45 behavior - this flow no longer downloads anything either.
...
**SUPERSEDED Round 46 (2026-09-09) - kept as history, do not delete.** This dialog used to re-download every folder it listed to establish a record. It no longer does: clicking its button now runs the same in-place `-Adopt` mechanism `install.ps1` already uses (ADOPT-SPEC.md section 2) - no download, no file write under `Interface\AddOns`, just a recorded folder/source id/`.toc` version exactly as it already sits on disk. "Take over"/"Taking over" is retired along with the behavior it described; nothing in the SPA calls this action "adopt" either, since that word reads as jargon to a below-average-tech-savvy player. The button now says what actually happens: **"Manage."**

```
   Found 6 addons you already have
   Furphy can keep them updated. Nothing is downloaded
   or changed now - you'll just see an Update badge
   when a newer version comes out.

        [ Not now ]         [ Keep them updated ]
```

Title: "Found N addon(s) you already have" (singular: "Found 1 addon you already have"). Body: "Furphy can keep them updated. Nothing is downloaded or changed now - you'll just see an Update badge when a newer version comes out." Buttons, same footer position as before: "Not now" (unchanged `btn-ghost`, was "Skip") and **"Keep them updated"** - restyled from the old plain `btn-outline` to the app's primary button treatment (it is now a safe, reversible-by-nature action, not a bulk re-download, so it earns the visual weight the old "Skip"-adjacent outline button deliberately withheld). Runs the same job either button in this dialog has ever run - see "Server job kind" below - just against every listed folder at once. No id is ever shown to the player (no "(CF 12345)"/"(Wago)" suffix on anything in this dialog - there was never a per-source choice here to begin with, unlike the Settings section below).

**Post-install variant** (ADOPT-SPEC.md section 4.2, shipped Round 45 as the "adopted" mode of this same dialog, reached only right after `install.ps1` silently adopted something and never at the same moment as the above): title "Furphy found N addon(s) you already had" and body "They're already the way you like them, so Furphy left the files alone and is just keeping track of them from here. You'll see an Update badge here if a newer version ever comes out." are **unchanged by this round** - that copy was already accurate and already banned-term-clean. What changes: the "Not now" button is **hidden** in this mode (there is nothing to decline - `install.ps1` already finished adopting before this dialog ever opens), and the surviving "Got it" button is restyled to the same primary treatment "Keep them updated" gets above, so a player who sees both dialogs across two sessions doesn't read one as more/less trustworthy than the other from button weight alone.

**Server job kind (both modes above, and the Settings section below, all three routes):** every entry point in this file posts the new job kind `adopt` (server maps it to the CLI's `-Adopt <folder[]>` for the listed folders, per the active flavour - see ADOPT-SPEC.md section 2 for the CLI contract) instead of the old `add` kind. No download, no `/api/` traffic to CurseForge or Wago. The job panel shows it like any other job (section 4): title "Managing N addon(s)..." while it runs (mirrors "Updating N of M addons..."/"Checking..." - section 7's Job panel title row), and on completion a toast: **"Now keeping N addon(s) up to date - nothing was downloaded or changed."** (singular: "Now keeping 1 addon up to date - nothing was downloaded or changed."). The old download-based `add`-kind job (and its own "Installing..."/"Added" wording) is untouched and still used everywhere it always was - explicit installs from Get new addons, `-Add`, the drawer's "Also on CurseForge/Wago" switch - this change is scoped to the two Welcome-dialog buttons and the Settings section below, nowhere else.
...
- **Done state**: one clean per-addon list only (delete the old triple-duplication of raw log + pill list + live table). Failed rows get a plain-language reason mapped from the `phase` the failure occurred in, plus an inline **Retry** scoped to that single addon (re-posts the job with the server's existing single-target `sync`/`add`/`install` param — no new server logic needed):
  ...
  - GitHub addon needs a token (`failPhase: "checking-needs-token"`, Round 47 - GITHUB-SOURCE-SPEC.md 3.5/3.6.5; covers both "no token configured" and "the configured token still 404s" - GitHub gives no way to tell those apart) -> "This addon needs a GitHub token - paste it in Settings > GitHub addons."
  - GitHub outage while checking (`failPhase: "checking-network-github"`, Round 47 - the GitHub-source counterpart to the CurseForge/Wago line above, same "transient outage, not a permanent incompatibility" reasoning, including a persisted per-record rate-limit backoff skipping the next check's own network call) -> "Couldn't check for updates - GitHub might be having trouble"
...
**Round 47 (2026-09-10, GITHUB-SOURCE-SPEC.md sections 4.1-4.3/5.1) - adds one new Advanced group, "GitHub addons," everything else below is unchanged.** GitHub joins CurseForge/Wago as a third addon source (SPEC.md's own Round 47 section is the day-to-day summary; GITHUB-SOURCE-SPEC.md is the full design). The new group sits inside the same Advanced disclosure the removed CurseForge API key field used to live in - a token is exactly the kind of thing that belongs behind Advanced. See the "GitHub addons" bullet in 6.2 below for the row-level detail; `SETTINGS-SPEC.md` Row 21 is the copy source of truth, same convention as every other Advanced row.
...
- **"GitHub addons"** (new, Round 47 - GITHUB-SOURCE-SPEC.md sections 4.1-4.3/5.1): a token field (password-style, show/hide + Save + Remove) for guilds that gate their GitHub-hosted addons behind a shared token, plus an "Add an addon from GitHub" inline input + Add button. Intro paragraph and info-tip tooltip, both verbatim: "Some guilds share addons as private GitHub releases and give you a token. It stays on this PC and is only ever sent to GitHub." A status line above the token field reads "No token saved." or "Token saved, ending in <last 4 chars>." - live-computed from the settings view's `hasGithubToken`/`githubTokenHint` fields, never the raw token (which the server never returns). Save with an empty field is a no-op with an inline nudge, not a silent clear - Remove is the only way to clear. "Get new addons" gets a matching small "From a GitHub link" entry opening the identical add flow. Exact strings and the token-field markup ship in `SETTINGS-SPEC.md` Row 21.
...
- ~~**"Folders Furphy doesn't manage yet"** (unchanged heading, sentence, and Scan/Take-over/Delete behavior) - the group heading and the Scan button each get a new tooltip; per-result "Take over" tooltips are corrected to say plainly that the addon is re-downloaded and its folder overwritten from the matched source (CurseForge or Wago) - an earlier draft claiming "it doesn't touch the files themselves" was false and is not shipped.~~ **SUPERSEDED Round 46 (2026-09-09)** - kept as history, do not delete; that correction was right for what the row did at the time (a real re-download), and the row's whole premise has now changed instead of just its wording. Heading and helper sentence both change this round, so no wording here is "unchanged" any longer, and the group's own scope note below (section 9) reflects the new behavior too.
- **"Addons Furphy isn't managing yet"** (renamed this round from "Folders Furphy doesn't manage yet" - Round 46, 2026-09-09; same position, same Scan/Delete mechanics, same `Components.Tooltip` component) - helper sentence rewritten to "These are already in your AddOns folder. Furphy can keep them updated without changing anything now." (was "Folders in your AddOns folder that Furphy doesn't manage yet."), stating the new no-download behavior up front rather than leaving a player to discover it by hovering a tooltip. Every recognizable result (a folder whose own `.toc` names a CurseForge or Wago id) now shows exactly **one** button, **"Manage"** - the old three-way "Take over (CF <id>)"/"Take over (Wago)"/"Take over" (manual-ID input + button) collapses into this single action, since the new mechanism (ADOPT-SPEC.md section 2's `-Adopt`) records whatever id the folder's own `.toc` already declares and never needs the player to type one in - manual ID entry is **removed** from this row entirely, not just relabeled (typing an unverified id here used to steer a real download to that id; there is no download left for it to steer). A result with no recognizable id in its own `.toc` (today's "No title found"/"No details found" row) is no longer offered any action beyond Delete - it moves under its own sub-heading, **"Furphy can't tell what these are, so it leaves them alone:"**, separating "Furphy can manage this with one click" from "Furphy genuinely doesn't know what this is" rather than showing both in one undifferentiated list. New **bulk button**, "Manage all (N)", next to Scan - runs the same job as clicking every recognizable row's own "Manage" button at once (mirrors the Welcome dialog's own bulk action, section 2.4); shown only when at least one recognizable result exists, same visibility rule "Update all" already uses when there is nothing to update. Tooltips: heading keeps its existing wording ("Addons installed by unzipping a download, or with another addon manager, before Furphy knew about them.") - still accurate, no change; Scan button's tooltip drops the retired term ("Looks only - nothing changes until you click Manage or delete a result." - was "...until you take over or delete a result"); the per-row "Manage" tooltip is now one sentence regardless of source, since the action itself no longer differs by source ("Furphy will start keeping this addon updated, exactly as it is now - nothing is downloaded or changed."); "Manage all (N)" gets the same sentence pluralized to the group ("Furphy will start keeping these N addons updated, exactly as they are now - nothing is downloaded or changed."). Exact strings and the `aria-label`s that replace "More about Take over (...)" ship in `SETTINGS-SPEC.md` section 2, Group 7 (Row 14) - that document is the copy source of truth, same convention as every other Advanced row.
...
| Settings > Untracked | "Folders in AddOns that aren't tracked by the manager." | "Folders in your AddOns folder that Furphy doesn't manage yet." - **SUPERSEDED Round 46 (2026-09-09), see the two rows below** |
| Settings > Untracked heading - Round 46, superseding this section's own prior row | "Folders Furphy doesn't manage yet" | "Addons Furphy isn't managing yet" |
| Settings > Untracked intro - Round 46 | "Folders in your AddOns folder that Furphy doesn't manage yet." | "These are already in your AddOns folder. Furphy can keep them updated without changing anything now." |
| Settings > Untracked, per-row action - Round 46 | "Take over (CF <id>)" / "Take over (Wago)" / "Take over" + manual ID field | "Manage" (one button, no id shown, no manual-ID input) |
| Settings > Untracked, bulk action - Round 46, new | (none) | "Manage all (N)" |
| Settings > Untracked, unrecognized-folder group - Round 46, new | (listed inline with everything else, same manual-ID "Take over" offered) | Own sub-heading: "Furphy can't tell what these are, so it leaves them alone:" (Delete only, no manage action offered) |
| Job completion toast, this flow only - Round 46, new | (an "Installed"/"Added" style toast, since the job used to be a real install) | "Now keeping N addon(s) up to date - nothing was downloaded or changed." |
...
| First-run dialog title | "Found addons in your AddOns folder" | "Found N addons in your AddOns folder" - **SUPERSEDED Round 46 (2026-09-09), see below** |
| First-run dialog body | "Furphy found {n} addon(s) it can manage. Adopting reinstalls each one from its source (CurseForge or Wago) so Furphy can keep it updated from here on." | "Furphy can start managing these - it re-downloads each one so it can keep them updated from now on." - **SUPERSEDED Round 46 (2026-09-09), see below** |
| First-run dialog button | "Adopt all (n)" | "Take over all (n)" - **SUPERSEDED Round 46 (2026-09-09), see below** |
| First-run dialog title - Round 46, supersedes the row above | "Found N addons in your AddOns folder" | "Found N addon(s) you already have" |
| First-run dialog body - Round 46, supersedes the row above | "Furphy can start managing these - it re-downloads each one so it can keep them updated from now on." | "Furphy can keep them updated. Nothing is downloaded or changed now - you'll just see an Update badge when a newer version comes out." |
| First-run dialog buttons - Round 46, supersedes the row above | "Skip" / "Take over all (n)" | "Not now" / "Keep them updated" (no count in the button text; restyled to the app's primary treatment) |
...
| First-run dialog, per flavour - Round 19, multi-flavour only (title pattern updated Round 46, flavour-insertion mechanics unchanged) | "Found 6 addons in your AddOns folder" | "Found 4 addons you already have in your Classic Era AddOns folder" (flavour name inserted only when more than one flavour's dialog shows in sequence) |
...
| WoW/AddOns folder paths, ad filter, addons Furphy isn't managing yet (renamed Round 46 from "untracked folders"), backup/restore, maintenance buttons, diagnostics, About | Settings > Advanced (collapsed by default, one click away) |
...
- **Confirm-dialog wording** (uninstall single/bulk, delete-unmanaged-folder, import, force-reinstall) — already plain and consequence-first per every Map pass; left as-is. (The "clear-key" dialog listed here in earlier rounds no longer exists — its whole feature was removed 2026-09-04, Expansion E23.)
...
- ~~**"Adopt" -> "Take over" rename (sections 2.4, 7, 11) governs the SPA only.** `install.ps1`'s own unattended first-run step (the one that runs automatically during install, before the app is ever opened) is a separate console/WinForms surface, not the SPA, and uses its own plain wording specified in `ADOPT-SPEC.md` section 3.5 - deliberately not "Take over," since that step no longer re-downloads anything (it only records existing addons as managed, unchanged on disk). Section 11's SPA banned-term checklist does not apply to `install.ps1`'s console/wizard output; do not flag its different wording as an inconsistency.~~ **SUPERSEDED Round 46 (2026-09-09)** - kept as history, do not delete; this bullet described a real, deliberate gap between the two surfaces at the time (ADOPT-SPEC.md section 0 spells out why: Round 45 changed only `install.ps1`'s unattended step, not the SPA's opt-in one). That gap is now closed - see the item below.
- **The SPA's opt-in "Manage" flow (Settings > "Addons Furphy isn't managing yet," and both first-run Welcome dialog buttons) now behaves the same as `install.ps1`'s own step: in place, no download, via the shared `-Adopt` CLI mechanism (ADOPT-SPEC.md section 2).** The two surfaces still don't share one wording source (the SPA is JS string literals gated by `tests\static\Test-BannedTerms.ps1`; `install.ps1`'s console/wizard output is PowerShell strings gated by its own console-flow wording tests, ADOPT-SPEC.md section 6.5) - but as of Round 46 both are held to the same banned-term list (section 11): neither may say "take over," "taking over," "reinstall," "untracked," or "adopt" in anything a player reads. Do not flag the continued existence of two separate string sources as an inconsistency; do flag either surface using a word from that list as one.
...
- [ ] No visible UI text (labels, buttons, chips, toasts, dialog copy, empty states, tooltips) contains any of: "project id", "file id", "release type", "toc", "interface version", "compat" (as a header/label — the word "compatible"/"compatibility" in plain sentences is fine), "digest", "stale", "stale-minor", "adopt"/"adopting", "untracked", "keyless", "indexed" (search badge), "instawow-data", "addon-radar.com", "HTTP 200" or any other raw status code, "Port" (in visible About). (`checkAddonVersion` dropped from this list Round 17 — the field/section it named no longer exists anywhere in the app, so there's nothing left for the term to leak from.)
- [ ] **(Round 46, 2026-09-09)** No visible UI text describing the "Addons Furphy isn't managing yet" section or either Welcome-dialog button contains "take over," "taking over," or "reinstall"/"reinstalling" - neither term describes what the action does any more (see sections 2.4/6.2). This does **not** rename the unrelated, still-genuinely-download-based "Force reinstall all" button (Settings > Backup & troubleshooting, `SETTINGS-SPEC.md` Row 17) or the CLI's own `-Only <id> -Force` flag - both still perform a real reinstall and keep their existing labels; do not flag either as a regression. No CurseForge/Wago id (numeric or slug) is shown anywhere in the "Manage"/"Manage all"/Welcome-dialog copy - the manual-ID input that used to sit next to an unrecognized row's "Take over" button is removed, not just relabeled.
...
- [ ] **(Round 46)** Clicking "Manage" (a single row), "Manage all (N)" (Settings), or "Keep them updated"/"Got it" (either Welcome-dialog mode) never produces a network request to CurseForge or Wago (watch the Browser pane's network log during the click) - the job completes instantly against local `.toc` data only, and the toast reads "Now keeping N addon(s) up to date - nothing was downloaded or changed." with correct singular/plural. A folder with no recognizable id in its own `.toc` shows under "Furphy can't tell what these are, so it leaves them alone:" with a Delete button only - no "Manage" button, no manual-ID text field, anywhere on that row.
