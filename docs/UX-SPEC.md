...
Word budget (visible chrome text, excludes table/row data): My Addons ≤ 25 words · Browse ≤ 20 words before results · Settings Essentials ≤ 60 words total. Anything over budget moves to Advanced or a per-item menu.
...
3. **Demote, don't delete.** Every existing capability stays reachable — in a per-row menu, a "Details" disclosure, or the single Advanced settings drawer — nothing is removed except two items that were pure jargon-restating-jargon with zero player value (the Compat hover tooltip, the Indexed/Live-match badge).
...
One page, two tiers: **Essentials** (always expanded, 3 groups, ≤ 60 words total) and one collapsed **Advanced** disclosure holding everything else. No more 10 flat equal-weight cards.
...
- "Auto-update when launching WoW" — toggle (shortened label, same setting).
- Zero helper prose — both are self-explanatory single-line toggles.

**Appearance**
- Theme: Lofi Night (default) / Dark / Light / Vaporwave — unchanged, unchanged prominence (hard constraint, Eric's favorite).
- Density: Comfortable / Compact — unchanged.
- Zero helper prose.

**CurseForge key (optional)**
- One line: "Not required — everything works without it. A free key adds CurseForge's own search and addon descriptions."
- Collapsed by default to a single row "Add a CurseForge key" that expands the field + Show/Hide + Test + Save + Clear in place when clicked (today's permanently-expanded 4-button section costs space most players never use).
...
- **WoW's out-of-date warning** — replaces the old 4-sentence, cvar-named-in-the-heading paragraph. Label: "Load out-of-date addons at character select." Value: "Off (they'll load anyway)." One line: "This is a WoW setting — change it in-game, not here." Read-only, informational, three lines total instead of 70+ words.
...
- **Ad filter** (native host only) — "Filter ads and trackers in the CurseForge tab", OFF by default (hard constraint, unchanged), existing one-sentence disclosure kept verbatim: "Off by default. CurseForge is ad-funded and pays addon authors from that revenue; enabling this is your choice." Plain-Edge-mode message ("Available in the Furphy desktop window.") unchanged.
...
| Settings > Game | "Out-of-date addon loading (checkAddonVersion)" + 4-sentence paragraph | "Load out-of-date addons at character select" / "Off (they'll load anyway)" / "This is a WoW setting — change it in-game, not here." |
...
| Settings > Maintenance | "Refresh CurseForge catalogue now" | "Refresh the addon list from CurseForge" |
...
| WoW/AddOns folder paths, checkAddonVersion block, ad filter, untracked folders, backup/restore, maintenance buttons, diagnostics, About | Settings > Advanced (collapsed by default, one click away) |
...
- **Lofi Night** default theme, its pixel-art cityscape and cats — untouched, unchanged, still first in the theme list (hard constraint, Eric's favorite).
- **Dark / Light / Vaporwave** — all stay fully selectable, unchanged.
- **Works fully without a CurseForge API key** — every flow in this spec (install, update, pin, rollback, search) assumes keyless is the default, fully functional path; the key only ever adds richer metadata/search, never gates a capability.
- **Both sources** — CurseForge and Wago Addons stay fully supported.
- **Native WebView2 host window**, `curseforge://` link handling, and the ad filter (OFF by default) — all stay, only relocated/deduplicated per §5 and §6.
...
- **Confirm-dialog wording** (uninstall single/bulk, delete-untracked-folder, clear-key, import, force-reinstall) — already plain and consequence-first per every Map pass; left as-is.
...
- [ ] No visible UI text (labels, buttons, chips, toasts, dialog copy, empty states, tooltips) contains any of: "project id", "file id", "release type", "toc", "interface version", "compat" (as a header/label — the word "compatible"/"compatibility" in plain sentences is fine), "digest", "stale", "stale-minor", "adopt"/"adopting" (renamed to "take over"), "untracked" (renamed to "doesn't manage yet"), "keyless", "indexed" (search badge), "instawow-data", "addon-radar.com", "checkAddonVersion", "HTTP 200" or any other raw status code, "Port" (in visible About).
- [ ] Lofi Night remains the default theme with its cityscape/cats art unchanged; Dark/Light/Vaporwave all still selectable; every new pill/bar/badge renders correctly (readable contrast, correct color) in all four themes.
