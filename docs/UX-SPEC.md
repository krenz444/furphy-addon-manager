...
- A deliberate, Eric-requested exception to this group's zero-prose rule, scoped to exactly those two lines on this one toggle — every other Essentials row (Round 18's three below included) stays prose-free.
- **Round 18 — background updates (new):** "Update addons in the background" — toggle; while on, an interval `<select>` appears directly beneath it (Every hour / Every 2 hours / Every 4 hours / Every 8 hours / Once a day). "Start with Windows" — a separate, independent toggle (turning it off never touches the background-updates toggle; turning it ON while background updates is currently off is the one coupled case — flips both in a single action, since a start-with-Windows entry that starts nothing running would be pointless to the player). One muted status line below all three, plainly worded, no jargon: "Background updates off" / "Running — last check HH:MM: everything up to date" / "Running — updated N at HH:MM" / "Running — N failed at HH:MM" / "Waiting — WoW is running". This status line is the ONLY prose permitted on these three rows — no per-toggle explainer sentences, matching this group's normal zero-prose rule everywhere except the auto-update carve-out above.

**Appearance**
- **Theme — Round 18 (2026-09-04): replaced the Vaporwave/Lofi Night/Dark/Light segmented toggle with a swatch grid**, one small color-preview tile per theme, 15 total — **Arcane Library (the default as of Round 19, 2026-09-05 — cats mixed with Warcraft-flavored fantasy, dark by default, readability-first per Eric's request; superseding Round 17's Vaporwave-default standing decision — see `THEMES-SPEC.md` section 7)**, Vaporwave, Lofi Night, Dark, Light, plus ten more options (Terminal Green, Arctic Ice, Art Deco, Alpine Dawn, Matcha, Desert Night, Tokyo Rain, Brushed Steel, Aurora Sky, Strawberry Cream — see `THEMES-SPEC.md` for the full design brief on each). Click (or Enter/Space on a focused tile) selects a theme immediately, no separate "Apply" step; arrow keys move focus between tiles without changing the applied theme, Home/End jump to the first/last tile. Zero helper prose — a labelled `radiogroup` and the checked tile's own visual state (ring + checkmark) are the only affordance, matching this group's existing zero-prose rule; no name-your-theme paragraph, no picker instructions.
- Density: Comfortable / Compact — unchanged.
- Zero helper prose (Appearance itself is unaffected by the Updates-group carve-outs above).
...
- [ ] Arcane Library is the default theme (Round 19, superseding Round 17's Vaporwave-default); Vaporwave's and Lofi Night's own art/tokens are unchanged, just no longer default; Vaporwave/Dark/Light/Lofi Night all still selectable; every new pill/bar/badge renders correctly (readable contrast, correct color) in every theme.
