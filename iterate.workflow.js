...
LIVE WAGO REGRESSION (mandatory, real addons.wago.io, at most 12 Wago requests): on the main test server, GET /api/wago/search?q=bonus+roll&sort=popular must return total 4 with slug bonusrollpreview among the items (Wago drops the search term for unknown sort values - the server must not forward them); GET /api/wago/search?q=bonus+roll&sort=name must return the same items alphabetically; GET /api/wago/addons/bonusrollpreview must return addon.id "nKW1zLNE"; GET /api/wago/addons/bonusrollpreview/releases must return releases whose first item has label, stability, size, supported_retail_patches and a changelog; POST /api/jobs {kind:"add", source:"wago", slug:"bonusrollpreview"} must reach done with an Installed row and a record with source "wago" and wagoId "nKW1zLNE"; a plain sync job must report it Up-to-date; POST /api/jobs {kind:"remove", projectId:"wago:bonusrollpreview"} must remove it. Offline fixtures do NOT satisfy this item.
